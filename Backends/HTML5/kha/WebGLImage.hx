...
class WebGLImage extends Image {
	...
	override public function at(x: Int, y: Int): Color {
		...
		var r = data.data[y * (image.width | 0) * 4 + x * 4];
		var g = data.data[y * (image.width | 0) * 4 + x * 4 + 1];
		var b = data.data[y * (image.width | 0) * 4 + x * 4 + 2];
		var a = data.data[y * (image.width | 0) * 4 + x * 4 + 3];
