	...
	override public function drawString(text: String, x: Float, y: Float) {
		...
		if (image.width > 0) {
			...
			for (i in 0...text.length) {
				var q;
				if (fontGlyphs != null)
					q = webfont.kravur._get(fontSize, fontGlyphs).getBakedQuad(backedQuadCache, fontGlyphs.indexOf(text.charCodeAt(i)), xpos, ypos);
				else
					q = webfont.kravur._get(fontSize).getBakedQuad(backedQuadCache, text.charCodeAt(i) - 32, xpos, ypos);
