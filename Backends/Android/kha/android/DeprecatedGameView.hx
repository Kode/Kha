...
class DeprecatedGameView extends SurfaceView implements SurfaceHolderCallback {
	...
	/*override*/
	public function onKeyDown(keyCode: Int, msg: KeyEvent): Bool {
		if (keyCode == KeyEvent.KEYCODE_BACK && msg.isAltPressed())
			return true; // Circle button on Xperia Play
		return thread.keyDown(keyCode);
	}

	/*override*/
	public function onKeyUp(keyCode: Int, msg: KeyEvent): Bool {
		return thread.keyUp(keyCode);
	}

	/*override*/
	public function onWindowFocusChanged(hasWindowFocus: Bool) {}
