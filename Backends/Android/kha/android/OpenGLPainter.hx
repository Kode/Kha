...
class OpenGLPainter extends kha.Painter {
	...
	override public function drawImage2(img: kha.Image, sx: Float, sy: Float, sw: Float, sh: Float, dx: Float, dy: Float, dw: Float, dh: Float) {
		if (bufferIndex + 1 >= bufferSize || (lastTexture != null && img != lastTexture))
			drawBuffer();
		...
		// mTriangleVertices.position(0);
		mTriangleVertices.put(x1);
		mTriangleVertices.put(y1);
		mTriangleVertices.put(cast(-1.0, Single));
		mTriangleVertices.put(u1);
		mTriangleVertices.put(v1);
		mTriangleVertices.put(x2);
		mTriangleVertices.put(y1);
		mTriangleVertices.put(cast(-1.0, Single));
		mTriangleVertices.put(u2);
		mTriangleVertices.put(v1);
		mTriangleVertices.put(x1);
		mTriangleVertices.put(y2);
		mTriangleVertices.put(cast(-1.0, Single));
		mTriangleVertices.put(u1);
		mTriangleVertices.put(v2);
