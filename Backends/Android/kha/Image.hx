...
class Image implements Canvas implements Resource {
	...
	public function new(width: Int, height: Int, format: TextureFormat, renderTarget: Bool, depthAndStencil: DepthStencilFormat) {
		...
		if (renderTarget) {
			...
			GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, realWidth, realHeight, 0, GLES20.GL_RGBA,
				format == TextureFormat.RGBA128 ? GLES20.GL_FLOAT : GLES20.GL_UNSIGNED_BYTE, null);
			...
			switch (format) {
				case L8:
					GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_LUMINANCE, realWidth, realHeight, 0, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE,
						null);
				case RGBA128:
	...
	function setup_oesExtension(): Int {
		...
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D, GLES20.GL_TEXTURE_MIN_FILTER, GLES20.GL_LINEAR);
		GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES11Ext.GL_DEPTH_STENCIL_OES, realWidth, realHeight, 0, GLES11Ext.GL_DEPTH_STENCIL_OES,
	...
	static function convertFramebufferStatus(status: Int): String {
		if (status == GLES20.GL_FRAMEBUFFER_COMPLETE)
			return "complete";
		else if (status == GLES20.GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT)
			return "incomplete attachments";
		else if (status == GLES20.GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT)
			return "incomplete missing attachments";
		else if (status == GLES20.GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS)
			return "incomplete dimensions";
		else if (status == GLES20.GL_FRAMEBUFFER_UNSUPPORTED)
			return "invalid combination of attachments";
		else
			return "unknown";
	}

	static function logFramebufferStatus(status: Int) {
		var message = convertFramebufferStatus(status);
		trace('framebuffer status "${message}"');
	}

	@:allow(kha.LoaderImpl)
	static function createFromFile(filename: String): Image {
		try {
	...
	static function createFramebuffer(): Int {
		var framebuffers = new NativeArray<Int>(1);
	...
	public function unlock(): Void {
		...
		switch (myFormat) {
			case L8:
				GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, width, height, GLES20.GL_LUMINANCE, GLES20.GL_UNSIGNED_BYTE,
			...
			case RGBA32:
				GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, width, height, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE,
					ByteBuffer.wrap(bytes.getData()));
			default:
				GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, width, height, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE,
					ByteBuffer.wrap(bytes.getData()));
