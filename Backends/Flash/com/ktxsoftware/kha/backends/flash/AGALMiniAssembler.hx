    ...
    public function assemble( mode:Context3DProgramType, source:String, verbose:Bool = false ):ByteArray
    {
      ...
      while (i < lng && _error == "")
      {
        ...
        while (j < Std.int(regLength))
        {
		  ...
		  reg = ~/\[.*\]/ig;
          var relreg : String = "";
          if (reg.match(regs[j])) {//(relreg.length > 0) {
          ...
          //var idxmatch:Array<String> = isRelative ? relreg.match( ~/\d+/ ) : regs[j].match( ~/\d+/ );
          var regidx:UInt = 0;
          ...
          if ( isRelative )
          {
            ...
            if ( verbose )
              trace( "RELATIVE: type="+reltype+"=="+relname+" sel="+relsel+"=="+selmatch+" idx="+regidx+" offset="+reloffset ); 
          }
