...
class VertexBuffer {
	...
	var vertexCount: Int;
	var myStride: Int;
	var myStructure: kha.graphics4.VertexStructure;
	...
	public function new(vertexCount: Int, structure: kha.graphics4.VertexStructure, usage: Usage, instanceDataStepRate: Int = 0, canRead: Bool = false) {
		...
		vertexBuffer = kha.flash.graphics4.Graphics.context.createVertexBuffer(vertexCount, myStride,
			usage == Usage.DynamicUsage ? Context3DBufferUsage.DYNAMIC_DRAW : Context3DBufferUsage.STATIC_DRAW);
	...
	public function set(): Void {
		...
		for (i in index...8)
			kha.flash.graphics4.Graphics.context.setVertexBufferAt(i, null);
	}
