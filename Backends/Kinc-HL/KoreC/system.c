...
void hl_kore_log(vbyte *v) {
	kinc_log(KINC_LOG_LEVEL_INFO, (char *)v);
}

double hl_kore_get_time(void) {
	return kinc_time();
}

int hl_kore_get_window_width(int window) {
	return kinc_window_width(window);
}

int hl_kore_get_window_height(int window) {
	return kinc_window_height(window);
}

vbyte *hl_kore_get_system_id(void) {
	return (vbyte *)kinc_system_id();
}

void hl_kore_vibrate(int ms) {
	kinc_vibrate(ms);
}

vbyte *hl_kore_get_language(void) {
	return (vbyte *)kinc_language();
}

void hl_kore_request_shutdown(void) {
	kinc_stop();
}

void hl_kore_mouse_lock(int windowId) {
	kinc_mouse_lock(windowId);
}

void hl_kore_mouse_unlock(void) {
	kinc_mouse_unlock();
}

bool hl_kore_can_lock_mouse(void) {
	return kinc_mouse_can_lock();
}

bool hl_kore_is_mouse_locked(void) {
	return kinc_mouse_is_locked();
}

void hl_kore_show_mouse(bool show) {
	if (show) {
		kinc_mouse_show();
	}
	else {
...
bool hl_kore_system_is_fullscreen(void) {
	return false; // kinc_is_fullscreen();
}

void hl_kore_system_request_fullscreen(void) {
	// kinc_change_resolution(display_width(), display_height(), true);
}

void hl_kore_system_exit_fullscreen(int previousWidth, int previousHeight) {
	// kinc_change_resolution(previousWidth, previousHeight, false);
}

void hl_kore_system_change_resolution(int width, int height) {
	// kinc_change_resolution(width, height, false);
}

void hl_kore_system_set_keepscreenon(bool on) {
	kinc_set_keep_screen_on(on);
}

void hl_kore_system_load_url(vbyte *url) {
	kinc_load_url((char *)url);
}

// const char* getGamepadId(int index);

vbyte *hl_kore_get_gamepad_id(int index) {
	return NULL;
...
void hl_kore_register_keyboard(vclosure *keyDown, vclosure *keyUp, vclosure *keyPress) {
...
void hl_kore_register_mouse(vclosure *mouseDown, vclosure *mouseUp, vclosure *mouseMove, vclosure *mouseWheel) {
...
void hl_kore_register_pen(vclosure *penDown, vclosure *penUp, vclosure *penMove) {
...
void hl_kore_register_gamepad(vclosure *gamepadAxis, vclosure *gamepadButton) {
...
void hl_kore_register_surface(vclosure *touchStart, vclosure *touchEnd, vclosure *touchMove) {
...
void hl_kore_register_sensor(vclosure *accelerometerChanged, vclosure *gyroscopeChanged) {
...
void hl_kore_register_callbacks(vclosure *foreground, vclosure *resume, vclosure *pause, vclosure *background, vclosure *shutdown) {
...
void hl_kore_register_dropfiles(vclosure *dropFiles) {
	// todo: string convert
...
void hl_kore_register_copycutpaste(vclosure *copy, vclosure *cut, vclosure *paste) {
...
const char *hl_kore_video_format(void) {
