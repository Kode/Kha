...
class Compute {
	...
	public static function setMatrix(location: kha.compute.ConstantLocation, matrix: FastMatrix4): Void {
		kore_compute_set_matrix(location._location, matrix._00, matrix._10, matrix._20, matrix._30, matrix._01, matrix._11, matrix._21, matrix._31,
	...
	public static function setMatrix3(location: kha.compute.ConstantLocation, matrix: FastMatrix3): Void {
		kore_compute_set_matrix3(location._location, matrix._00, matrix._10, matrix._20, matrix._01, matrix._11, matrix._21, matrix._02, matrix._12,
			matrix._22);
	}
	...
	public static function setTexture(unit: TextureUnit, texture: Image, access: Access) {
		if (texture._texture != null)
			kore_compute_set_texture(unit._unit, texture._texture, access);
		else
			kore_compute_set_target(unit._unit, texture._renderTarget, access);
	}

	public static function setSampledTexture(unit: TextureUnit, texture: Image) {
		if (texture._texture != null)
			kore_compute_set_sampled_texture(unit._unit, texture._texture);
		else
			kore_compute_set_sampled_target(unit._unit, texture._renderTarget);
	}
	...
	public static function setSampledCubeMap(unit: TextureUnit, cubeMap: CubeMap) {
		if (cubeMap._texture != null)
			kore_compute_set_sampled_cubemap_texture(unit._unit, cubeMap._texture);
		else
			kore_compute_set_sampled_cubemap_target(unit._unit, cubeMap._renderTarget);
	}
